def cb_input_hid(
    ...
    """
    HID input callback using the 'hid' library (https://pypi.org/project/hid/).

    Args:
        states_input: numpy array to store input states
        verbose: print debug information
        input_keys: list of input keys to map
        vendor_id: USB vendor ID (required)
        product_id: USB product ID (required)
        remap_trigger: remap trigger axes from [-1,1] to [0,1]
        blocking: use blocking reads (not recommended)
        report_id: specific HID report ID to read (optional)
    """
    ...
    try:
        ...
        print('hid not available, install with: pip install hid')
        return None
    ...
    gamepad_parsers = {
        ...
        # Logitech gamepads (common layout)
        'logitech': {
            'axes': {
                'ABS_X': (0, 1, 'uint8'),
                'ABS_Y': (1, 1, 'uint8'),
    ...
    def detect_parser(vid, pid, report_data=None):
        """Detect appropriate parser based on vendor/product ID and report format"""
    ...
    def find_device():
        """Find a HID device by enumerating available devices"""
        devices = hid.enumerate()

        # If vendor_id/product_id specified, try to find exact match
        if vendor_id is not None and product_id is not None:
            for dev_info in devices:
                if dev_info['vendor_id'] == vendor_id and dev_info['product_id'] == product_id:
                    return dev_info
            return None

        # Otherwise, look for gamepad-like devices
        for dev_info in devices:
    ...
    def init_device():
        ...
        if vendor_id is None or product_id is None:
            ...
            if dev_info is None:
                print('No HID device found. Please specify vendor_id and product_id.')
                return None

            vid = dev_info['vendor_id']
            pid = dev_info['product_id']
            print(f'Auto-detected device: VID=0x{vid:04x}, PID=0x{pid:04x}')
        else:
            vid = vendor_id
            pid = product_id
        ...
        try:
            ...
            try:
                ...
                print(f'Device opened (info unavailable: {e})')
            ...
            # Detect parser type
            parser_type = detect_parser(vid, pid)
    ...
    def cb():
        ...
        try:
            ...
            # Auto-detect parser format if PS controller (check parser name in gamepad_parsers)
            parser_name = None
            ...
            if parser_name and 'playstation' in parser_name:
                new_parser_type = detect_parser(vendor_id or 0x054c, product_id or 0, data)
            ...
            if verbose:
                ...
                print(f'HID report ({len(data)} bytes): {" ".join(f"{b:02x}" for b in data)}')
                if len(data) == 10 and parser_name == 'playstation_short':
                    print(
                        f'  Byte 5 (face+dpad): 0x{data[5]:02x} (lower/dpad: 0x{data[5]&0x0F:01x}, upper/face: 0x{data[5]&0xF0:02x})'
                    )
                    print(f'  Byte 6 (shoulder): 0x{data[6]:02x}')
                    print(f'  Byte 8 (L2): 0x{data[8]:02x}')
                    print(f'  Byte 9 (R2): 0x{data[9]:02x}')
                elif len(data) >= 64 and parser_name == 'playstation_long':
                    print(
                        f'  Byte 9 (face+dpad): 0x{data[9]:02x} (lower/dpad: 0x{data[9]&0x0F:01x}, upper/face: 0x{data[9]&0xF0:02x})'
                    )
                    print(f'  Byte 10 (shoulder): 0x{data[10]:02x}')
    ...
    # Store close callback as an attribute
    cb.close = cb_close
...
if __name__ == '__main__':
    ...
    # Example usage with Logitech gamepad
    # test_cb_input(lambda si: cb_input_hid(si, vendor_id=0x046d, product_id=0xc534))
    test_cb_input(cb_input_hid)
