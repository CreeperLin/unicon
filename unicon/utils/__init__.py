...
def cmd(*args, capture_output=False, timeout=None, **kwds):
    ...
    res = subprocess.run(
        run_args, shell=False, capture_output=capture_output, **({} if capture_output else stdargs), **kwds
    )
...
def parse_robot_def(robot_def):
    ...
    urdf_path, mjcf_path = [
        (x if x is None or x.startswith('/') else os.path.join(asset_dir, x)) for x in asset_paths
    ]
